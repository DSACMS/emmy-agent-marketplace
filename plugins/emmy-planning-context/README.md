...
Requirement ingestion with approval gate:

```text
Use $emmy-requirements-ingest to ingest requirements from Confluence page 1112978748. Present the review packet first and wait for my approval before writing.
```
...
- No silent requirements writes. Requirement persistence must use
  `emmy-requirements-ingest`, target Requirements Registry `1399165621` and
  child pages under it, and wait for explicit human approval.
- No arbitrary URL, repository, Jira issue, or Knowledge Store ingestion for
  requirements in v1.
...
For Confluence write details, use the relevant write-safety reference:

- `skills/emmy-planning-ingest/references/write-safety.md`
- `skills/emmy-requirements-ingest/references/write-safety.md`
